...
describe('End to End UI test for calculating factorial of a number', () => {
  beforeEach(() => {
    cy.visit("/")
  })
  ...
  positiveData.forEach(data => {
    it(`displays correct result when using valid data. Input: ${data.input}`, () => {
  ...
  negativeData.forEach(data => {
    it(`displays error message when using invalid data. Input: ${data.input}`, () => {
      HomePage.enterAnInteger(data.input)
      HomePage.calculateFactorial()
